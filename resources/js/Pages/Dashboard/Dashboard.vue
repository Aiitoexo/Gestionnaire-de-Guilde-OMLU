<template>
    <div class="w-screen h-screen p-10 flex space-x-5">
        <div class="w-8/12 h-full bg-white shadow-2xl rounded-lg overflow-hidden border border-gray-50">
            <ul class="flex w-full nav">
                <resource-button @click="isActive(category.name)" :category="category.name" v-for="category in all_categories" :class="active === category.name ? 'text-gray-900 bg-gray-300' : 'bg-gray-800'"></resource-button>
            </ul>
            ...
            <div class="contain w-full overflow-y-auto relative">
                ...
                <div v-for="category in all_categories" class="px-5">
                    <div v-if="active === category.name" v-for="resource in category.resource" :key="resource.id" class="space-x-8 p-5 shadow-lg bg-gray-50 mb-8 rounded-xl border border-gray-50 flex items-center grid grid-cols-12">
        ...
        <div class="w-4/12 h-full bg-white shadow-2xl rounded-lg overflow-hidden border border-gray-200">
            <div class="h-1/6 w-full grid grid-cols-1 bg-gray-50 pb-3">
