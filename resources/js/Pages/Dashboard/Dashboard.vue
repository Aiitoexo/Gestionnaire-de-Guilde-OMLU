<template>
    <default-layout>
        <div class="w-full flex relative gap-x-8">
            <div class="w-6/12 rounded-md text-white overflow-hidden">
                <div class="space-y-3 mb-3" v-for="discord_users in all_main_roles" :key="discord_users.id">
                    <div>
                        <p class="bg-purpleDark-900 text-3xl font-medium py-3 text-center text-purple-300"
                           style="mask-size: 100% 100%;mask-image: url('data:image/png;base64,')">
                            {{ discord_users.name_role }}
                        </p>
                    </div>

                    <div class="bg-black rounded-md bg-opacity-60 py-4 space-y-4">
                        <div class="flex gap-4 h-12 grid grid-cols-12 px-6" v-for="user in discord_users.user">
                            <div class="col-span-4 flex items-center bg-gray-800"
                                 style="mask-size: 100% 100%;mask-image: url('data:image/png;base64,')">
                                <p class="text-xl pl-6 text-purple-300">{{ user.pseudo }}</p>
                            </div>

                            <div class="col-span-4 flex items-center justify-center">
                                <p class="text-xl font-sans text-purple-300">{{ user.id_discord_user }}</p>
                            </div>

                            <div class="col-span-4 h-full w-full">
                                <button class="h-full w-full bg-gray-800 rounded text-white text-2xl" @click="userDiscord(user.id)">
                                    Info
                                </button>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

            <div v-if="DiscordUser !== null" class="w-6/12 h-full sticky top-24">
                <info-user
                    :discord_user="DiscordUser"
                    :contribution_active="contribution_active"
                    :contribution_gold="contribution_gold"
                    :contribution_item="contribution_item">
                </info-user>
            </div>
        </div>
    </default-layout>
</template>

<script>
import DefaultLayout from "../../Layouts/DefaultLayout";
import InfoUser from "./Components/InfoUser";

export default {
    components: {
        DefaultLayout,
        InfoUser
    },
    props: ['all_main_roles', 'all_discord_users', 'contribution_active'],
    name: "Dashboard",
    data() {
        return {
            DiscordUser: null,
            contribution_gold: null,
            contribution_item: null
        }
    },
    methods: {
        userDiscord(id) {
            let discord_user = null
            let contrib_gold = null
            let contrib_item = null

            this.all_discord_users.forEach(user => {
                if (user.id === id) {
                    discord_user = user
                }
            })

            this.contribution_active[0]['gold_contribution'].forEach(user => {
                if (user.id === id) {
                    contrib_gold = user
                }
            })

            this.contribution_active[0]['item_contribution'].forEach(user => {
                if (user.id === id) {
                    contrib_item = user
                }
            })

            this.DiscordUser = discord_user
            this.contribution_gold = contrib_gold
            this.contribution_item = contrib_item
        }
    },
}
</script>

<style scoped>
.rang {
    background-color: #151515;
}
</style>
